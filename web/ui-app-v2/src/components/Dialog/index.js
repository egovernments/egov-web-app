...
handleChange=(event)=>{
  ...
  render() {
    ...
    const actions=close?([<FlatButton label="close" primary={true} onClick={this.handleClose}/>]):
                  ok?  ([<FlatButton label="ok" primary={true} onClick={this.handleOk}/>,
    ...
    return (
      ...
      <FlatButton
        ...
        <div style={{ background:'lightblue' }}>{<h2 style={{ color:'black' }}>Example Dialog</h2>}
        {(ok || reset) && <div><label style={{ color:'black' }}>Enter Your Name:</label><input type="text" value={inputname} onChange={this.handleChange}></input></div>}
        </div>
        </Dialog>
