...
const getEventSLA = (eventTime) => {
  ...
  let sla ;
  if(days > 30)
   sla=<Label label="CS_SLA_MONTH" dynamicArray={[Math.floor((days / 30))]} fontSize={12}/> ;
  else if(days > 7)
   sla=<Label label="CS_SLA_WEEK" dynamicArray={[Math.floor((days / 7))]} fontSize={12} />;
  else if(days>=1)
   sla= <Label label="CS_SLA_DAY" dynamicArray={[Math.floor(days)]} fontSize={12} />;
  else{
     if( (days % 1) * 24 > 1 )
      sla=<Label label="CS_SLA_TIME" dynamicArray={[Math.ceil((days % 1) * 24)]} fontSize={12} />;
     else{
       if((days % 1) * 24 * 60 > 1 )
         sla=<Label label="CS_SLA_MINUTE" dynamicArray={[Math.ceil((days % 1) * 24*60)]} fontSize={12} />;
        else
        <Label label="CS_SLA_NOW"  fontSize={12} />;
     }
  }
