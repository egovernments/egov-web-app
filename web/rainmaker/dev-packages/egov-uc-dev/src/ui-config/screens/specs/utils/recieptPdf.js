...
/* Required data format: Object
   Note: All the values will be fetched from the API data in the above format.
   This part will be done when the API is ready.
   Usage: import {generateReciept} from "../../utils/receiptPdf" Add this in the file wherever it is needed.
*/
...
export const generateReciept = async (state, dispatch, type) => {
  pdfMake.createPdf(receiptData).open();
};

export const generateCitizenReciept = async (state, dispatch, type) => {
  pdfMake.createPdf(citizenRecieptData).open();
};
