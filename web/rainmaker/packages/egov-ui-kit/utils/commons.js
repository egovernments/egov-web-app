...
var returnSLAStatus = exports.returnSLAStatus = function returnSLAStatus(slaHours, submittedTime) {
  ...
  if (daysCount < 0) {
    return Math.abs(daysCount) === 1 ? "CS_COMMON_OVERDUE_BY " + Math.abs(daysCount) + " CS_COMMON_DAY" : "CS_COMMON_OVERDUE_BY " + Math.abs(daysCount) + " CS_COMMON_DAYS";
  } else {
    return Math.abs(daysCount) === 1 ? Math.abs(daysCount) + " CS_COMMON_DAY_LEFT" : Math.abs(daysCount) + " CS_COMMON_DAYS_LEFT";
  }
};
