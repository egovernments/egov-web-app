...
var displayLocalizedStatusMessage = (exports.displayLocalizedStatusMessage = function displayLocalizedStatusMessage(
  status
) {
  return status ? statusToLocalisationKeyMapping[status.toLowerCase()] : "";
});
var transformById = (exports.transformById = function transformById(
  payload,
  id
) {
...
var hyphenSeperatedDateTime = (exports.hyphenSeperatedDateTime = function hyphenSeperatedDateTime(
  d
) {
  return d;
});
...
var isFieldEmpty = (exports.isFieldEmpty = function isFieldEmpty(field) {
  ...
  if (
    (typeof field === "undefined"
      ? "undefined"
      : (0, _typeof3.default)(field)) !== "object"
  ) {
...
var persistInLocalStorage = (exports.persistInLocalStorage = function persistInLocalStorage(
  obj
) {
...
var fetchFromLocalStorage = (exports.fetchFromLocalStorage = function fetchFromLocalStorage(
  key
) {
  return window.localStorage.getItem(key) || null;
});

var getRequestUrl = (exports.getRequestUrl = function getRequestUrl(
  url,
  params
) {
...
var mapCompIDToName = (exports.mapCompIDToName = function mapCompIDToName(
  IDObj,
  compID
) {
...
var getDateFromEpoch = (exports.getDateFromEpoch = function getDateFromEpoch(
  epoch
) {
...
var getBodyClassFromPath = (exports.getBodyClassFromPath = function getBodyClassFromPath(
  path
) {
  var bodyClass = path
    .split("/")
...
var prepareFormData = (exports.prepareFormData = function prepareFormData(
  form
) {
  ...
  return Object.keys(formFields).reduce(function(formData, fieldKey) {
    ...
    if (file) {
      value = ((form.files && form.files[fieldKey]) || []).map(function(
        fieldFile
      ) {
...
var getTranslatedLabel = (exports.getTranslatedLabel = function getTranslatedLabel(
  labelKey,
  localizationLabels
) {
  ...
  if (localizationLabels && localizationLabels.hasOwnProperty(labelKey)) {
    ...
    if (
      translatedLabel &&
      (typeof translatedLabel === "undefined"
        ? "undefined"
        : (0, _typeof3.default)(translatedLabel)) === "object" &&
...
var getCityNameByCode = (exports.getCityNameByCode = function getCityNameByCode(
  code,
  cities
) {
...
var isImage = (exports.isImage = function isImage(url) {
  ...
  var imageType =
    ...
    urlParts.length &&
    urlParts[0].split(".") &&
    urlParts[0].split(".").length &&
    urlParts[0].split(".").pop();
...
var getTransformedStatus = (exports.getTransformedStatus = function getTransformedStatus(
  status
) {
...
var getNameFromId = (exports.getNameFromId = function getNameFromId(
  obj,
  id,
  defaultValue
) {
...
var getPropertyFromObj = (exports.getPropertyFromObj = function getPropertyFromObj(
  obj,
  id,
  property,
  defaultValue
...
var returnSLAStatus = (exports.returnSLAStatus = function returnSLAStatus(
  slaHours,
  submittedTime
) {
  var millsToAdd = slaHours * 60 * 60 * 1000;
  var toBeFinishedBy = millsToAdd + submittedTime;
  var daysCount = dateDiffInDays(
    new Date(Date.now()),
    new Date(toBeFinishedBy)
  );
  if (daysCount < 0) {
    return Math.abs(daysCount) === 1
      ? "Overdue by " + Math.abs(daysCount) + " day"
      : "Overdue by " + Math.abs(daysCount) + " days";
  } else {
    return Math.abs(daysCount) === 1
      ? Math.abs(daysCount) + " day left"
      : Math.abs(daysCount) + " days left";
  }
});

var getLatestCreationTime = (exports.getLatestCreationTime = function getLatestCreationTime(
  complaint
) {
...
var transformLocalizationLabels = (exports.transformLocalizationLabels = function transformLocalizationLabels(
  localizationLabels
) {
...
var getTenantForLatLng = (exports.getTenantForLatLng = (function() {
  var _ref2 = (0, _asyncToGenerator3.default)(
    /*#__PURE__*/ _regenerator2.default.mark(function _callee2(lat, lng) {
            ...
            switch ((_context2.prev = _context2.next)) {
              case 0:
                queryObjList = [
                  { key: "lat", value: lat },
                  { key: "lng", value: lng },
                  { key: "tenantId", value: _common2.default.tenantId }
                ];
                response = void 0;
                ...
                return (0, _api.httpRequest)(
                  _endPoints.TENANT.POST.URL,
                  _endPoints.TENANT.POST.ACTION,
                  queryObjList
                );
...
var findLatestAssignee = (exports.findLatestAssignee = function findLatestAssignee(
  actionArray
) {
...
var transformComplaintForComponent = (exports.transformComplaintForComponent = function transformComplaintForComponent(
  complaints,
  role,
  employeeById,
  citizenById,
  ...
  var transformedComplaints = Object.values(complaints.byId).map(function(
    complaintDetail,
    index
  ) {
    var filedUserName =
      complaintDetail &&
      complaintDetail.citizen &&
      complaintDetail.citizen.name;
    var isFiledByCSR =
      complaintDetail &&
      complaintDetail.actions &&
      complaintDetail.actions[complaintDetail.actions.length - 1].by &&
      complaintDetail.actions[complaintDetail.actions.length - 1].by.split(
        ":"
      )[1] &&
      complaintDetail.actions[complaintDetail.actions.length - 1].by.split(
        ":"
    ...
    return {
      header: getPropertyFromObj(
        complaints.categoriesById,
        complaintDetail.serviceCode,
        "serviceCode",
        "NA"
      ),
      ...
      images: fetchImages(complaintDetail.actions).filter(function(
        imageSource
      ) {
        return isImage(imageSource);
      }),
      complaintStatus:
        complaintDetail.status && getTransformedStatus(complaintDetail.status),
      rawStatus: complaintDetail.status && complaintDetail.status,
      address: complaintDetail.address
        ? complaintDetail.address
      ...
      reassignRequestedBy:
        complaintDetail.status === "reassignrequested"
          ? getPropertyFromObj(
              employeeById,
              complaintDetail.actions[0].by.split(":")[0],
              "name",
              "NA"
            )
          : "NA",
      ...
      latestActionTime:
        complaintDetail &&
        complaintDetail.actions &&
        getLatestAction(complaintDetail.actions),
      submittedBy: filedUserName
        ? isFiledByCSR
          ? filedUserName + " (Citizen Service Desk)"
          : filedUserName
        : "NA",
      citizenPhoneNumber:
        complaintDetail &&
        complaintDetail.citizen &&
        complaintDetail.citizen.mobileNumber,
      assignedTo:
        complaintDetail &&
        getPropertyFromObj(
          employeeById,
          findLatestAssignee(complaintDetail.actions),
          "name",
          "NA"
        ),
      employeePhoneNumber:
        employeeById &&
        employeeById[findLatestAssignee(complaintDetail.actions)]
          ? employeeById[findLatestAssignee(complaintDetail.actions)]
              .mobileNumber
          : defaultPhoneNumber,
      status:
        role === "citizen"
          ? displayStatus(
              ...
              complaintDetail.assignee,
              complaintDetail.actions.filter(function(complaint) {
...
var mergeMDMSDataArray = (exports.mergeMDMSDataArray = function mergeMDMSDataArray(
  oldData,
  newRow
) {
        ...
        function _callee3$(_context3) {
          while (1) {
            switch ((_context3.prev = _context3.next)) {
              case 0:
                (url = dataFetchConfig.url),
                  (action = dataFetchConfig.action),
                  (requestBody = dataFetchConfig.requestBody),
                  (queryParams = dataFetchConfig.queryParams);
                _context3.prev = 1;
                _context3.next = 4;
                return (0, _api.httpRequest)(
                  url,
              ...
              case 4:
                ...
                dropdownData = boundary
                  ? _jsonpath2.default.query(
                      payloadSpec,
                      dataFetchConfig.dataPath
                    )
                  : (0, _get2.default)(payloadSpec, dataFetchConfig.dataPath);
                ddData =
                  dropdownData &&
                  dropdownData.reduce(function(ddData, item) {
              ...
              case 10:
                ...
                dispatch(
                  (0, _actions2.toggleSnackbarAndSetText)(true, message, true)
                );
