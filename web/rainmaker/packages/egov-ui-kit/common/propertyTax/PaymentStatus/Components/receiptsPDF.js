...
var generateReceipt = function generateReceipt(
  role,
  details,
  generalMDMSDataById,
  receiptImageUrl,
  ...
  var transform = function transform(value, masterName) {
    if (value) {
      return generalMDMSDataById && generalMDMSDataById[masterName]
        ? generalMDMSDataById[masterName][value].name
        : "";
    } else {
      return "NA";
    }
    ...
    case "pt-reciept-citizen":
      ...
      var getFloorDetails = function getFloorDetails() {
        ...
        if (units && units.length) {
          ...
          dataRow.push({
            text: "Built Area/Total Annual Rent",
            style: "receipt-assess-table-header"
          });
          ...
          units &&
            units.map(function(unit) {
              ...
              dataRow.push(
                transform(unit.usageCategoryMajor, "UsageCategoryMajor")
              );
              dataRow.push(
                transform(unit.usageCategorySubMinor, "UsageCategorySubMinor")
              );
      ...
      var getOwnerDetails = function getOwnerDetails(ownerArray, noOfColumns) {
        var transformedArray = ownerArray.map(function(item, index) {
          return [
            {
              text:
                "Owner " + (ownerArray.length > 1 ? index + 1 : "") + " Name",
              border: borderKey,
        ...
        content: [
          ...
          },
          {
            ...
            table: {
              body: [
                [
                  {
                    text: "Existing Property ID:",
                    border: borderKey,
                    style: "receipt-table-key"
                  },
                  {
                    text: details.existingPropertyId || "NA",
                    border: borderValue
                  },
                  {
                    text: "Property Tax Unique ID:",
                    border: borderKey,
                    style: "receipt-table-key"
                  },
                  ...
                  {
                    text: propertyDetails[0].assessmentNumber || "",
                    border: borderValue
                  }
                ]
              ]
            ...
            table: {
              ...
              body: [
                [
                  ...
                  {
                    text: propertyDetails[0].landArea + " sq yards" || "",
                    border: borderValue
                  },
                  {
                    text: "Property Type:",
                  ...
                  },
                  {
                    text: propertyDetails[0].propertySubType
                      ? transform(
                          propertyDetails[0].propertySubType,
                          "PropertySubType"
                        )
                      : transform(
                          propertyDetails[0].propertyType,
                          "PropertyType"
                        ),
                    border: borderValue
                  }
          ...
          { text: "Note:", alignment: "left", style: "receipt-footer" },
          {
            ol: [
              {
                text:
                  "Payment received by cheque/demand draft shall be subject to realization.",
                margin: [0, 0, 0, 5]
              },
              {
                text: "This document is not a proof of Property Ownership.",
                margin: [0, 0, 0, 5]
              },
              {
                text:
              ...
              {
                text:
                  "Assessment & Payment is subject to verification/Scrutiny by competitive authority.",
                margin: [0, 0, 0, 5]
              }
            ],
            alignment: "left",
  ...
  data &&
    _pdfmake2.default.createPdf(data).download(details.ReceiptNo + ".pdf");
};
