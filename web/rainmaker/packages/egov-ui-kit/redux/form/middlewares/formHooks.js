            ...
            dependants.forEach(function (item) {
              if (fields[item.fieldKey].dataFetchConfig) {
                if (fields[item.fieldKey].boundary) {
                  fields[item.fieldKey].dataFetchConfig.dataPath = "$.TenantBoundary.*.boundary[?(@parent.hierarchyType.code===\"" + item.hierarchyType + "\"&&@.label==\"City\"&&@.code==\"" + value + "\")]..children[?(@.label==\"Locality\")]";
