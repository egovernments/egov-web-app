...
class ComplaintResolved extends Component {
  ...
  onSubmit = (e) => {
    if (!this.props.isFormValid) {
      e.preventDefault();
      this.props.toggleSnackbarAndSetText(true, "Please upload photo or write comments", true);
    }
  };
...
const mapStateToProps = (state) => {
  ...
  const formValidArray = fields
    ? Object.keys(fields).reduce((result, current) => {
        if (current !== "action") {
          const formValid = fields[current].value || (files && files.media && files.media.length) ? true : false;
          result.push(formValid);
        }
        return result;
      }, [])
    : [];
  const isFormValid = formValidArray && formValidArray.length ? (formValidArray.indexOf(true) === -1 ? false : true) : false;
  return { isFormValid };
};
