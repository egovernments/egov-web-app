...
class Playground extends React.Component {
    ...
    propTypeUsage = (data) => {
        ...
        // console.log(getPlotAndFloorFormConfigPath(propUsageType, propType));
        // console.log(`egov-ui-kit/config/forms/specs/${path}`);
        if (formConfigPath["hasPlot"]) {
            ...
            Object.keys(configPlot["fields"]).map((item) => {
                var path = configPlot["fields"][item]["jsonPath"];
                var value = get(data, path);
                // console.log("In plot " + item + ":" + value + "#" + configPlot["fields"][item]["value"]);
            ...
            // Object.keys(configPlot["fields"]).map((item) => {
            //     console.log("In plot " + item + ":" + configPlot["fields"][item]["value"]);
            //     // console.log(configPlot);
        ...
        if (formConfigPath["hasFloor"]) {
            ...
            for (var unitIndex = 0; unitIndex < units.length; unitIndex++) {
                ...
                var keyName = `floorDetails_${floorNo}_unit_${unitIndex}`;
                ...
                Object.keys(configFloor["fields"]).map((item) => {
                    var path = configFloor["fields"][item]["jsonPath"];
                    path = path.replace(/units\[[0-9]\]/g, "units[" + unitIndex + "]");
                    var valueInJSON = get(data, path);
                    // console.log("Path:" + path);
                    // console.log("In floor " + item + ":" + valueInJSON);
                    configFloor["fields"][item].value = valueInJSON;

                });
                dictFloor[keyName] = configFloor;
            ...
            console.log(dictFloor);

            console.log(dictCustomSelect);
        }
        ...
        Object.keys(configBasic["fields"]).map((item) => {
            var path = configBasic["fields"][item]["jsonPath"];
            var valueInJSON = get(data, path);
            // console.log("Path:" + path);
            // console.log("In floor " + item + ":" + valueInJSON);
